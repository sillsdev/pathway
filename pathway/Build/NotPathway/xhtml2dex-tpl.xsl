...
<!-- #############################################################
    ...
    #
    # Created:     2009/11/23
    # Changes from xhtml2dex.xsl - remove namespace def for x, put variables for params, use stringLength
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" 
    ...
    <xsl:param name="ver" select="'%(Vernacular)s'"/>
    <xsl:param name="l1" select="'%(Language1)s'"/>
    <xsl:param name="l2" select="'%(Language2)s'"/>
        ...
        <xsl:choose>
            ...
            <xsl:when test="@class='entry'">
                <xsl:for-each select="div[@class='pictureRight']">
                    ...
                    <xsl:text>}</xsl:text>
                    <xsl:text>{</xsl:text>
                    <xsl:value-of select="div/span[@class='pictureLabel']/span"/>
                    <xsl:text>}</xsl:text>
                </xsl:for-each>
